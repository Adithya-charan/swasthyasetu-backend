...
public class PrescriptionService {
    ...
    public PrescriptionService(PrescriptionRepository prescriptionRepository, AppointmentService appointmentService, UserService userService) {
    ...
    public Prescription issuePrescription(@NonNull UUID doctorId, @NonNull IssuePrescriptionRequest req) {
        DoctorProfile doctor = userService.getDoctorProfile(doctorId);
        if (doctor.getUser().getAccountStatus() != AccountStatus.ACTIVE || doctor.getVerificationStatus() != VerificationStatus.VERIFIED) {
            throw new ResourceConflictException("Doctor is not active or verified to issue prescriptions");
        }
